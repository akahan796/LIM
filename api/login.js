...
const COOKIE_NAME = 'lim_auth';          // must match middleware.js / logout.js
...
export default async function handler(request) {
  ...
  if (token && role) {
    ...
    headers.append('Set-Cookie', `lim_role=${role}; Path=/; Secure; SameSite=Lax`);
    const editTok = process.env.EDIT_TOKEN;
    if (role === 'editor' && editTok) {
      headers.append('Set-Cookie', `lim_edit=${editTok}; Path=/; HttpOnly; Secure; SameSite=Lax`);
    } else {
      headers.append('Set-Cookie', `lim_edit=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0`);
    }
