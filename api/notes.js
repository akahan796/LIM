...
export default async function handler(request){
  ...
  const authed = cookie(request, 'lim_auth') === process.env.SESSION_TOKEN && !!process.env.SESSION_TOKEN;
  ...
  if (request.method === 'POST'){
    ...
    const canWrite = editToken
      ? cookie(request, 'lim_edit') === editToken
      : cookie(request, 'lim_role') === 'editor';
